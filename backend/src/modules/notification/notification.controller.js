  ...
  async (
    ...
    try {
      ...
      return res.json({
        ...
        message:
          `${result.markedCount} notification(s) marked as read`,
      });
    } catch (error) {
      console.error(
    ...
    try {
      ...
      if (!resolvedAudience) {
        return res.status(400).json({
          success: false,
          message:
            "Notification audience is required",
        });
      }
      ...
      if (
        ...
        if (
          ...
          return res.status(403).json({
            success: false,
            message:
              "Admin can send only holiday and exam timetable notifications to students.",
          });
        }
      }
      ...
      if (
        ...
        if (
          !TEACHER_STUDENT_TYPES.includes(
            notificationType
          )
        ) {
          return res.status(403).json({
            success: false,
            message:
              "This notification type is not allowed for teacher-to-student notifications.",
          });
        }
      }
